          ...
          {mangasDB.length > 0 && (
            <TouchableOpacity style={styles.hero} onPress={() => setSelectedManga(mangasDB[0])}>
              ...
              <View style={styles.heroText}>
                ...
                <View style={styles.heroBtn}><Text style={styles.heroBtnText}>LEER AHORA</Text></View>
              </View>
            </TouchableOpacity>
          )}
              ...
              <TouchableOpacity style={styles.continueCard} onPress={() => setSelectedManga(item)}>
                ...
                <View style={styles.progressContainer}>
                  <View style={styles.miniProgress}>
                    <View style={[styles.miniBar, { width: `${item.Progreso}%` }]} />
                  </View>
                  <Text style={styles.progressText}>{Math.round(item.Progreso)}%</Text>
                </View>
                <Text style={[styles.cardTitle, { color: isDarkMode ? 'white' : 'black' }]} numberOfLines={1}>{item.Titulo}</Text>
              </TouchableOpacity>
            )}
...
const styles = StyleSheet.create({
  ...
  // HERO MODIFICADO
  hero: { width: width - 40, height: 250, alignSelf: 'center', borderRadius: 20, overflow: 'hidden', backgroundColor: '#000' },
  heroImageContainer: { width: '100%', height: '100%', overflow: 'hidden' },
  heroImage: { 
    width: '100%', 
    height: '140%', // Un poco más alta para tener margen de movimiento
    resizeMode: 'cover', 
    transform: [{ translateY: 40 }] // ESTO BAJA LA IMAGEN PARA VER LAS CARAS
  }, 
  heroGradient: { position: 'absolute', left: 0, right: 0, top: 0, bottom: 0 },
  heroText: { position: 'absolute', bottom: 20, left: 20 },
  heroTitle: { color: 'white', fontSize: 26, fontWeight: 'bold', textShadowColor: 'rgba(0, 0, 0, 0.9)', textShadowOffset: {width: 2, height: 2}, textShadowRadius: 5 },
  heroBtn: { backgroundColor: 'white', paddingHorizontal: 15, paddingVertical: 8, borderRadius: 8, marginTop: 10, alignSelf: 'flex-start' },
  heroBtnText: { fontWeight: 'bold', fontSize: 14, color: '#000' },
  
  sectionTitle: { fontSize: 18, fontWeight: 'bold', marginTop: 30, marginBottom: 15 },
  continueCard: { marginRight: 15, width: 150 },
  continueImage: { width: 150, height: 85, borderRadius: 10, resizeMode: 'cover' },
  
  // PORCENTAJE AGREGADO
  progressContainer: { flexDirection: 'row', alignItems: 'center', marginTop: 8 },
  miniProgress: { flex: 1, height: 4, backgroundColor: '#333', borderRadius: 2, marginRight: 8 },
  miniBar: { height: '100%', backgroundColor: '#6C5CE7', borderRadius: 2 },
  progressText: { color: '#6C5CE7', fontSize: 10, fontWeight: 'bold' },

  stdCard: { marginRight: 15, width: 110 },
  stdImage: { width: 110, height: 160, borderRadius: 10, resizeMode: 'cover' },
  cardTitle: { marginTop: 8, fontSize: 12, fontWeight: '600' },
  
  detailHero: { width: '100%', height: 400, backgroundColor: '#000' },
  ...
  gridImage: { width: '100%', height: 220, borderRadius: 10, resizeMode: 'cover' }
});